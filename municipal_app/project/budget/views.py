...
@budget_blueprint.route('/budget/<types>', methods=['GET', 'POST'])
@login_required
@check_confirmed
def budget(types):
    if types in 'annuel':
        if 'open_api' in request.values:
            max_year = Budget_annuelle.query.filter_by(municipal_id=current_user.municipal_id).order_by(Budget_annuelle.year.desc()).first()
            api_data = get_api_data(request.values['r_id'], request.values['file_type'], request.values['link'], str(max_year.year), None, None)
            try:
        ...
        if request.method == 'POST' and 'file_types' in request.values:
            if 'file' not in request.files:
                flash(u'لا يوجد ملف', 'info')
        ...
        if Budget_annuelle.query.filter_by(municipal_id=current_user.municipal_id).first() and check_monthly_data("Depence"):
            csv_mensuelle_file('depence')
